...
async function sendTransactionWithKeystore(): Promise<Hash> {
  try {
    ...
    const keystore = await KeystoreUtils.loadKeystore(join(process.cwd(), keystorePath))
    ...
    const privateKey = await KeystoreUtils.decryptPrivateKey(keystore, keystorePassword)
