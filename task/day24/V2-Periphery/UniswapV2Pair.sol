...
contract UniswapV2Pair  {
    ...
    // 两个token在交易对中的储备量。两个token在交易对中的储备量。  已经由了token，那我们可以根据token地址来查询 这个pair合约地址 在token合约的 amount。那为什么pair还要用一个变量来记录呢？
    //   因为是为了防止有人恶意操作价格：通过闪电待借来一笔token0.reserve0,这样resere0数量增加，然后导致reserve0价格暴跌，然后作恶人用一点点reserve1买走价格低的reserve0。
    //   所以pair合约需要记录token的储备量，防止价格被恶意操控。
