...
contract ERC721 is IERC721, IERC721Metadata {
    ...
    function approve(address owner2, uint256 tokenId ) public override {
        ...
        require( owners_[tokenId]!=address(0) && owner2!=address(0), "tokenId is not exist or to is not valid");
        // if(owners_[tokenId]!=address(0)) { revert TokenNotExist();}
    ...
    function _transfer( address owner1, address from, address owner2, uint tokenId ) private {
        require(owner2 != address(0) && from == owner1, "transfer to the zero address or not owner");
    ...
    function _mint(uint tokenId, address owner1) public {
        require(owner1 != address(0) && owners_[tokenId] == address(0), "mint to zero address or token already minted");
